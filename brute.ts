...
type Result = { userId: string; species: string; eye: string; hat: string; shiny: boolean; stats: Record<StatName, number> }
...
for (let i = 0; i < maxAttempts; i++) {
  ...
  if (roll.rarity === targetRarity && (!shinyOnly || roll.shiny) && (!targetSpecies || roll.species === targetSpecies)) {
    ...
    console.log(`  stats: ${STAT_NAMES.map(s => `${s}=${roll.stats[s]}`).join(' ')}`)
    console.log()
...
if (results.length > 0) {
  ...
  for (const r of results) {
    ...
    console.log(`  ${r.userId}  =>  ${r.species} (${r.eye}) hat:${r.hat}${shinyTag}`)
  }
}
